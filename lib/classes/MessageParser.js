...
class MessageParser {
	...
	async run() {
		...
		const { status, commandName, args } = this.#parseContent({
			...
			prefix: databaseInstances.guild.prefix
		});
		if (!status)
			return;
	 ...
	 * @return {DatabaseInstances} Object with database instances.
	 *
	 ...
	 * @return {Promise<DatabaseInstances>}
	 * @this MessageParser
	 */
	#resolveInstances = async function (message) {
		return {
			...
			member: await GuildMember.findOrCreate({
				where: {
					id_guild: message.guild.id,
					id_user: message.author.id
				}
			})[0],
			channel: await GuildChannel.findOrCreate({
				where: {
 ...
 * @property {Guild} guild Guild database instance.
 * @property {User} user User database instance.
 * @property {GuildMember} member Guild member database instance.
 * @property {GuildChannel} channel Guild channel database instance.
 */
