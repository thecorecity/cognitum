...
const { fileExtension, createModuleLogger, checkObjectKeySafety } = require("../Utils");
const logger = createModuleLogger("commands");
const BaseCommand = require("../base/BaseCommand");
const BaseCategory = require("../base/BaseCategory");
...
class CommandsRegistry {
	 ...
	 * @type {Record<string, typeof BaseCommand>}
	 */
	static commands = {};
	...
	/**
	 * Map of categories.
	 * @type {Object<string, typeof BaseCategory>}
	 */
	static categories = {};
	...
	static async initialize() {
		...
		categoriesDirectory.forEach(filename => {
			...
			if (!checkObjectKeySafety(code)) {
				logger.error(`Invalid category code passed: ${code}!`);
				process.exit();
			}
	...
	static #loadCommandFromPath(requirePath) {
		...
		if (!checkObjectKeySafety(code)) {
			logger.error(`Invalid command name: ${code}!`);
			process.exit();
		}
		if (!CommandClass.validateMetaInformation()) {
			logger.error(`Command validation failed for ${code} command!`);
			process.exit();
		}
		if (this.commands.hasOwnProperty(code)) {
			logger.error("Command with following code already available: " + code + "!");
			process.exit();
		}
		this.commands[code] = CommandClass;
		logger.debug(`Command ${code} successfully loaded!`);
		if (!CommandClass.aliasesAvailable())
			return;
		logger.debug(`Loading aliases for ${code}...`);
		const aliases = CommandClass.getAliases();
		aliases.forEach(alias => {
			if (!checkObjectKeySafety(alias)) {
				logger.error(`Invalid alias name: ${alias}!`);
				process.exit();
			}
			if (this.aliases.hasOwnProperty(alias)) {
				logger.error(`Warning! Alias ${alias} already used by command ${this.aliases[alias]}!`);
				process.exit();
			}
			this.aliases[alias] = code;
			logger.debug(`Alias registered: ${alias} => ${code}.`);
		});
	}
