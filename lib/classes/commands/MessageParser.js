...
import { GuildChannelModel, GuildMemberModel, GuildModel, MessageStatisticsModel, UserModel } from "../Database";
import { createModuleLogger } from "../Utils";
import ConfigManager from "../ConfigManager";
import { ChannelType } from "discord-api-types/v10";
import StringableError from "../errors/base/StringableError";
import GuildsCachingTask from "../statistics/tasks/GuildsCachingTask";
...
export default class MessageParser {
	...
	async resolve() {
		...
		try {
			this.#maybeCreateCachingTask(databaseInstances.guild);
		} catch (e) {
	...
	/**
	 * @param {GuildModel} guildInstance
	 */
	#maybeCreateCachingTask(guildInstance) {
		const cacheDate = guildInstance.cache_timestamp;
		const borderDate = new Date();
		borderDate.setDate(borderDate.getDate() - 14);

		if (
			// Skip if guild was cached earlier
			cacheDate && cacheDate instanceof Date && cacheDate < borderDate

			// Skip if delay is applied
			|| MessageParser.#cacheGenerationTaskPushed.hasOwnProperty(guildInstance.id)
			&& MessageParser.#cacheGenerationTaskPushed[guildInstance.id] > new Date().getTime()
		) {
			return;
		}

		const delay = new Date();
		delay.setHours(delay.getHours() + 6);
		MessageParser.#cacheGenerationTaskPushed[guildInstance.id] = delay;
	 ...
	 * @param {EmbeddableError | Error} error Error class.
	...
	static #onError(error, context) {
		...
		if (error instanceof StringableError)
			return error.toString();

		console.error(error);

		const unexpectedError = new EmbeddableError(
	...
	static #cacheGenerationTaskPushed = {};
}
