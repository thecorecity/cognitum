...
class BaseError extends Error {
	...
	toEmbed({ context, thumbnailMode }) {
		...
		const embedOptions = this._generateEmbedOptions(context);
		embed
			.setTitle(
				context.lang.get(
					this._generateEmbedTitle(),
					embedOptions
				)
			)
			.setDescription(
				context.lang.get(
					this._generateEmbedDescription(),
					embedOptions
				)
			);
		return embed;
