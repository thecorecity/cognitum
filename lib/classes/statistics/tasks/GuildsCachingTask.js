...
import { createModuleLogger, parseToBigIntOrDefault } from "../../Utils";
import { GuildChannelModel, GuildMemberModel, GuildModel } from "../../Database";
import StatisticsManager from "../StatisticsManager";

const logger = createModuleLogger("guildCachingTask");

export default class GuildsCachingTask extends BaseTask {
	/**
	 * @type {CognitumClient}
	 */
	#client;
	...
	async #generateCacheForGuild(id) {
		if (GuildsCachingTask.#isGuildLocked(id))
			return;

		GuildsCachingTask.#lockGuild(id);
		...
		GuildsCachingTask.#unlockGuild(id);
	}
	...
	static #isGuildLocked(guildId) {
		return this.#lockedGuilds[guildId] && this.#lockedGuilds[guildId] > new Date().getTime();
	}

	static #lockGuild(guildId) {
		const lockdownDelay = new Date();
		lockdownDelay.setHours(lockdownDelay.getHours() + 6);

		this.#lockedGuilds[guildId] = lockdownDelay;

		return lockdownDelay;
	}

	static #unlockGuild(guildId) {
		delete this.#lockedGuilds[guildId];
	}
