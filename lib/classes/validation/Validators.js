const CallerPermissionError = require("../errors/CallerPermissionError");
const BotPermissionError = require("../errors/BotPermissionError");
...
module.exports = {
	...
	botPermission({ context, options }) {
		checkPermission(
			context.message.guild.members.me,
			options,
			"bot"
		);
	},
	...
	arguments({ context, options = {} }) {
		...
		if (
			...
			&& context.args.length < options.min
		) {
		...
		if (
			...
			&& context.args.length > options.max
		) {
		...
		if (
			...
			for (let i = 0; i < args.length; i++) {
				...
				} else if (!["max", "exact"].includes(currentMode)) {
