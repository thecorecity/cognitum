...
class LogsProcessor extends BaseDiscordModule {
	 ...
	 */
	#proxyDiscordEvents() {
		this._client.on("guildMemberAdd", member => this.#emitter.emit("join", member));
		this._client.on("guildMemberRemove", member => this.#resolveMemberRemove(member));
		this._client.on("guildMemberUpdate", (prev, curr) => this.#resolveMemberUpdate(prev, curr));
		this._client.on("guildBanAdd", details => this.#resolveBanManagementEvent("ban", details));
		this._client.on("guildBanRemove", details => this.#resolveBanManagementEvent("unban", details));
		this._client.on("messageCreate", message => this.#resolveMessage(message));
		this._client.on("messageUpdate", (prev, curr) => this.#resolveMessageUpdate(prev, curr));
		this._client.on("messageDelete", message => this.#resolveMessageDelete(message));
	}
	...
	async #resolveBanManagementEvent(eventName, details) {
		if (!details.guild.me.permissions.has("VIEW_AUDIT_LOG"))
	...
	static #eventMessageRenderer = {
		...
		msgupdate: (prev, curr) => {
			...
			embed
				...
				.addField(
					lang.get("embed.logs.messageUpdate.info.name"),
					lang.get("embed.logs.messageUpdate.info.value", {
		...
		msgdelete: message => {
			...
			embed
				...
				.addField(
					lang.get("embed.logs.messageDelete.info.name"),
					lang.get("embed.logs.messageDelete.info.value", {
