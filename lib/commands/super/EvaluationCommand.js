...
class EvaluationCommand extends BaseCommand {
	async run() {
		/**
		 * Do not run evaluation command if user is not authorized super user.
		 */
		if (!this.#checkAccess()) {
			logger.warn(`${this.context.getMessage().author.tag} Tried to access eval command!`);
	...
	#checkAccess() {
		...
		return superUsers.includes(this.context.getMessage().author.id);
	}
