...
import {createModuleLogger, formatTimeString, parseToBigIntOrDefault} from "../../classes/Utils";
import {GuildChannelModel, GuildMemberModel, GuildModel} from "../../classes/Database";
	...
	async #generateVoiceList() {
		...
		if (!this.#isCachedOnly) {
			topVoice = (await new StatisticsManager(this.message).queryTopVoice({ dateStart: cachedTo }))
				.map(entry => ({
					id: entry[GuildMemberModel.name].id_user.toString(),
