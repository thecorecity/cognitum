...
import StatisticsCategory from "../../categories/StatisticsCategory";
import DefaultEmbed from "../../classes/embed/DefaultEmbed";
import {PermissionsBitField} from "discord.js";
import {formatTimeString, parseToBigIntOrDefault} from "../../classes/Utils";
import StatisticsManager from "../../classes/statistics/StatisticsManager";
import {GuildMemberModel} from "../../classes/Database";
import DiscordTimestamp from "../../classes/content/DiscordTimestamp";
...
export default class ProfileCommand extends BaseCommand {
	...
	async #generateProfileDescription() {
		...
		let maybeDate = ProfileCommand.#parseDateOrNull(this.context.models.guild.cache_timestamp);

		const userId = this.#targetMember.id;
		const statisticsManager = new StatisticsManager(this.message);

		const [messageStats] = await statisticsManager.queryTopMembers({ dateStart: maybeDate, entity: userId });
		const [voiceStats] = await statisticsManager.queryTopVoice({ dateStart: maybeDate, entity: userId });
		...
		return [
			...
			this.resolveLang("command.profile.parts.lastJoined", {
				joinDate: [
					new DiscordTimestamp(this.#targetMember.joinedAt, DiscordTimestamp.styleShortDate),
					new DiscordTimestamp(this.#targetMember.joinedAt, DiscordTimestamp.styleLongTime)
				].join(", ")
			}),
			this.resolveLang("command.profile.parts.accountCreated", {
				createDate: [
					new DiscordTimestamp(this.#targetMember.user.createdAt, DiscordTimestamp.styleShortDate),
					new DiscordTimestamp(this.#targetMember.user.createdAt, DiscordTimestamp.styleLongTime),
				].join(", ")
			}),
