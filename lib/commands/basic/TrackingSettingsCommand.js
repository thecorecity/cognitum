...
import {DocumentModel, GuildMemberModel, MessageStatisticsModel, VoiceStatisticsModel} from "../../classes/Database";
 ...
 */
export default class TrackingSettingsCommand extends BaseCommand {
	/**
	 * Run the command.
	...
	async #cleanupUserActivity() {
		return await TrackingSettingsCommand.removeUserActivity(
	...
	/**
	 * Drop all the caches for the target user if needed.
	 */
	#dropCachesForUser() {
		/** @type {CognitumClient<true>} */
		const client = this.context.message.client;

		client.voiceStatsManager.clearUserCache(this.message.author.id);
	}
	...
	static async removeUserActivity(userId) {
		...
		for (let memberInstance of membersList) {
			entriesRemoved += await MessageStatisticsModel.destroy({
				where: {
					id_member: memberInstance.id
				}
			});
			entriesRemoved += await VoiceStatisticsModel.destroy({
				where: {
					id_member: memberInstance.id
				}
			});
			entriesRemoved += await DocumentModel.destroy({
				where: {
					id_member: memberInstance.id
				}
			});
