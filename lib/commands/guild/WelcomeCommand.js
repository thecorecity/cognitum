...
class WelcomeCommand extends BaseCommand {
	...
	async toggleFeature(state) {
		await this.context.models.guild.update("welcome_mode", state ? 1 : 0);
	...
	async setRole(type, id) {
		...
		await this.context.models.guild.update(`welcome_${type}_role`, id);
		const reply = new DefaultEmbed(this.context, "guild");
		reply.setTitle(
	...
	async setWelcomeChannel(id) {
		...
		await this.context.models.guild.update("welcome_channel", id);
		const reply = new DefaultEmbed(this.context, "guild");
		reply.setTitle(
