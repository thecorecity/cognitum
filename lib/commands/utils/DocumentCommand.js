...
import OrderedList from "../../classes/content/OrderedList";
import HiddenCategory from "../../categories/HiddenCategory";
import CallerPermissionError from "../../classes/errors/CallerPermissionError";
import DocumentError from "../../classes/errors/commands/DocumentError";
import {DocumentModel, GuildMemberModel} from "../../classes/Database";
import {escapeMarkdown} from "../../classes/Utils";
import {PermissionsBitField} from "discord.js";

export default class DocumentCommand extends BaseCommand {

	/**
	 * Resolving command call.
	 * @return {Promise<DefaultEmbed|any>}
	...
	async run() {
		...
		if (this.args.length === 2) {
			...
			if ((command === "create" || command === "edit") && await this.#checkPermissions([PermissionsBitField.Flags.Administrator]))
				return await this.#pushDocument(param, command === "create");
			if (command === "delete" && await this.#checkPermissions([PermissionsBitField.Flags.Administrator]))
		...
		if (this.args.length >= 3) {
			...
			if (command === "rename" && await this.#checkPermissions([PermissionsBitField.Flags.Administrator]))
				return await this.#renameDocument(slug, content);
			if (command === "image" && await this.#checkPermissions([PermissionsBitField.Flags.Administrator]))
	...
	async #checkPermissions(permissions) {
		if (!this.message.member.permissions.has(permissions))
	 ...
	 * @throws {DocumentError} If this document is missing.
	 */
	...
	async #displayDocumentsList(pageNumber = 1) {
		...
		const { rows } = await DocumentModel.findAndCountAll({
			...
			limit: DocumentCommand.#pageSize,
			offset
		});
		...
		embed
			...
			.setFooter({
				text: this.resolveLang("command.doc.list.pagerFooter", {
					pageNumber
				})
			});
		return embed;
	...
	async #pushDocument(slug, isNew) {
		...
		if (isNew) {
			await DocumentModel.create({
				id_member: this.context.models.member.id,
				name: slug,
	 ...
	 * @throws {DocumentError} If user canceling action by himself.
	 * @throws {DocumentError} If timeout for new message happened.
	 */
	async #requestDocumentContent(isNew) {
		...
		embed
			...
			.setFooter({
				text: this.resolveLang("command.doc.pushDocumentFooter")
			});
		...
		try {
			...
			const messages = await this.message.channel.awaitMessages({
				filter: message => message.author.equals(this.message.author),
				time: 60000,
				max: 1,
				errors: ["time"]
			});
	 ...
	 * @throws {DocumentError} If document with current slug is not found.
	 */
	 ...
	 * @throws {DocumentError} If document missing.
	 */
	 ...
	 * @throws {DocumentError} If target document not founded.
	 */
